...
func main() {
	...
	accessKey := os.Getenv("CLEARML_API_ACCESS_KEY")
	secretKey := os.Getenv("CLEARML_API_SECRET_KEY")
	if accessKey == "" || secretKey == "" {
		fmt.Fprintln(os.Stderr, "CLEARML_API_ACCESS_KEY and CLEARML_API_SECRET_KEY must be set")
		os.Exit(2)
	}
	...
	c, err := client.NewClearMLClient(ctx, "terraform-provider-clearml/acceptance-cleanup", accessKey, secretKey, apiHost)
	if err != nil {
